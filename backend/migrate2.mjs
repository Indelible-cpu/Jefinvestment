...
const connStr = process.env.DATABASE_URL || 'postgresql://neondb_owner:npg_cZivLq7JXh4S@ep-wandering-flower-axpx499e-pooler.c-4.us-east-2.aws.neon.tech/neondb?sslmode=require';
...
async function main() {
  console.log('🚀 Starting migration via postgres client...\n');
...
async function seed() {
  ...
  try {
    // Create admin role
    const roleRes = await sql.unsafe(`
    ...
    const branchRes = await sql.unsafe(`
